...
<body>
	...
	<font size="4" color=00008B>LEMBAR DATA ANALISA UJI MUTU <b> KOMPONEN DARAH MERAH PEKAT (PRC-Leucodepleted)</b></font><br><br>
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="4" style="width:200px;">UDD PUSAT PMI</th>
  	    <th colspan="2" style="width:300px; height:30px;">LEMBAR DATA ANALISA UJI MUTU KOMPONEN DARAH MERAH PEKAT (PRC-Leucodepleted)</th>
	    <th rowspan="2" align="left" style="width:155px;">Halaman <br/>Nomor <br/>Versi <br/>Tanggal Berlaku <br/>Tanggal Kajiulang</th>
	    <th rowspan="2" align="left" style="width:190px;">: 1 dari 1 <br/>: UDDP-PM-L3-048 <br/>: 004 <br/>: 01/07/2020 <br/>: 01/07/2022</th>
	...
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`
		...
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                ...
                WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucodepleted' and q.`qc_status` like '$src_hasilqc%' and a.`asal_utd` like '$src_utd%'
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="2" colspan="3">Pemeriksaan Hematologi</th>
	    ...
	    <th rowspan="2">Volume<br/> 250 &plusmn 50 mL<br/> (200 - 300 mL)</th>
	...
	<tr style="background-color:mistyrose; font-size:12px; color:#000000;">
	    ...
	    <th>Hematokrit<br/> 50 - 70 %</th>
	    <th>Hemoglobin<br/> > 40 g/unit</th>
	    <th>Residual Leukosit<br/> < 5 x 10 <sup>6</sup> </th>
	...
	$sql="SELECT q.`qcchecker`, q.`qcuser`, q.`qctgl`, q.`jenis`, q.`nokantong`, q.`gol_darah`, q.`RhesusDrh`, q.`tglaftap`,
        q.`kadaluwarsa`, q.`berat_isi`, q.`volume`, q.`hemolisis`, q.`hemolisis_manual`, q.`hemoglobin`, q.`aerob`, q.`anaerob`, 
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`, q.`hct`, q.`leukosit`
                FROM `qc` q
                LEFT JOIN `registrasi_qc` a on q.`nokantong`=a.`nokantong`
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                LEFT JOIN utd s ON a.`asal_utd` = s.id
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucodepleted' 
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucodepleted' 
AND q.`volume` >= 230 AND q.`volume` <=301 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucodepleted' 
AND q.`hemolisis` <= 0.8 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like 
'%PRC Leucodepleted' AND q.`leukosit` <= 5 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
$leukosit1=mysql_num_rows($sqlwb5);
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucodepleted' 
AND q.`hct` >= 50 AND q.`hct` <= 70 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
$hematokrit1=mysql_num_rows($sqlwb8);

//hemoglobin
$sqlwb9=mysql_query("SELECT * FROM `qc` q LEFT JOIN `registrasi_qc` a on q.`nokantong`=a.`nokantong` 
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucodepleted' 
AND q.`hemoglobin` >= 40 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
$hemoglobin1=mysql_num_rows($sqlwb9);
...
$leukosit2= $persenhb * $leukosit1;

//persentase hasil hematokrit
$persenhema=$persenwb / $jumbariswb;
$hematokrit2= $persenhema * $hematokrit1;

//persentase hasil hemoglobin
$persenhema=$persenwb / $jumbariswb;
$hemoglobin2= $persenhema * $hemoglobin1;
...
<td align="center"><b><? echo number_format ($hematokrit2,2); ?>%</b></td>
<td align="center"><b><? echo number_format ($hemoglobin2,2); ?>%</b></td>
<td align="center"><b><? echo number_format ($leukosit2,2); ?>%</b></td>
...
<table align="right" border=1 cellpadding=4  style="border-collapse:collapse">
        <tr style="background-color:mistyrose; font-size:12px; color:#000000;">
	    <th align="left">Catatan : *Standar Kelulusan* <br/>Residual Leukosit/Unit = < 5 x 10<sup>6</sup></th>
	</tr><br>
</table>
...
<!--akhir petugas-->
	...
	<?
		if ($no!==0){
		?>
		<a href="pmiqc.php?module=cetak_rekap-leucodepleted&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_blue">Download Data Per UTD</a>
		<a href="pmiqc.php?module=cetak_rekap-leucodepleted-all&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_green">Download Semua Data</a>
		<?
		}
	?>
