...
if (isset($_POST['submit'])) {
    ...
    for ($i = 0; $i < count($_POST['nk']); $i++) {
        ...
        $bdrs_sql1 = "insert into registrasi_qc (nokantong, merk, produk, jenis_produk, volume,goldarah,rhesus,tgl,tglaftap,kadaluwarsa,tgl_pengolahan,petugas_terima,petugas_serah,asal_utd, suhu, jns_asal, catatan) values ('$pd[noKantong]','$pd[merk]','$pd[produk]','$pd[jenis]','$pd[volumeasal]','$pd[gol_darah]','$pd[RhesusDrh]','$tglSerahTerima','$pd[tgl_Aftap]','$pd[kadaluwarsa]','$pd[tglpengolahan]','$namauser','$v_petugas2','$asalUDD', '$suhu', '$jns_asal', '$catatan')";
        // echo $bdrs_sql1;
