...
while ($komp = mysql_fetch_assoc($a)) {
    ...
    <tr style="font-size:16px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'"
        ...
        <td><?= $lastChar == 'A' ? '-' : $beku; ?></td>
        <td><?= $lastChar == 'A' ? '-' : date('H:i', strtotime($komp['mulaiBeku'])) ?></td>
        <td><?= $lastChar == 'A' ? '-' : date('H:i', strtotime($komp['selesaiBeku'])) ?>
        </td>
        <td><?= $lastChar == 'A' ? '-' : $komp['bsuhu'] . '&deg;C'; ?></td>
        <td><?= $petugas; ?></td>
    </tr>
    <?php }
if ($no == "0") {
