...
<table class="list" border=1 cellpadding="5" cellspacing="5" width="100%" style="border-collapse:collapse">
    ...
    ?>
        <tr style="font-size:16px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'"
            ...
            <td><?= $komp['metode']; ?></td>
            <td><?= $putar; ?></td>
            <td><?= date('H:i', strtotime($komp['mulaiPutar'])) ?></td>
            <td><?= date('H:i', strtotime($komp['selesaiPutar'])) ?></td>
            <td><?= $pisah; ?></td>
            <td><?= date('H:i', strtotime($komp['mulaiPisah'])) ?></td>
            <td><?= date('H:i', strtotime($komp['selesaiPisah'])) ?></td>
            <td><?= strtolower($lastChar) == 'a' ? '-' : $beku; ?></td>
            <td><?= strtolower($lastChar) == 'a' ? '-' : date('H:i', strtotime($komp['mulaiBeku'])) ?></td>
            <td><?= strtolower($lastChar) == 'a' ? '-' : date('H:i', strtotime($komp['selesaiBeku'])) ?>
            </td>
            <td><?= strtolower($lastChar) == 'a' ? '-' : $komp['bsuhu'] . '&deg;C'; ?></td>
            <td><?= $petugas; ?></td>
        </tr>
    <?php }
    if ($no == "0") {
