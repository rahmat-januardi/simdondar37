...
<body>
	...
	<font size="4" color=00008B>LEMBAR DATA ANALISA UJI MUTU <b> KOMPONEN DARAH MERAH PEKAT (PRC-Leucoreduction)</b></font><br><br>
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="4" style="width:200px;">UDD PUSAT PMI</th>
  	    <th colspan="2" style="width:300px; height:30px;">LEMBAR DATA ANALISA UJI MUTU KOMPONEN DARAH MERAH PEKAT (PRC-Leucoreduction)</th>
	    <th rowspan="2" align="left" style="width:155px;">Halaman <br/>Nomor <br/>Versi <br/>Tanggal Berlaku <br/>Tanggal Kajiulang</th>
	    <th rowspan="2" align="left" style="width:190px;">: 1 dari 1 <br/>: UDDP-PM-L3-047 <br/>: 004 <br/>: 01/07/2020 <br/>: 01/07/2022</th>
	...
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`
		...
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                ...
                WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucoreduction' and q.`qc_status` like '$src_hasilqc%' and a.`asal_utd` like '$src_utd%'
	...
	<tr style="background-color:mistyrose; font-size:12px; color:#000000;">
	    ...
	    <th>Residual Lekosit<br/> < 1,2 x 10 <sup>9</sup> per unit </th>
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucoreduction' 
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC Leucoreduction' 
AND q.`volume` >= 200 AND q.`volume` <=301 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
'%PRC Leucoreduction' AND q.`leukosit` < 1.2 and a.`asal_utd` like '$src_utd%' ") or die(mysql_error());
...
<td align="center"><b><? echo number_format ($hbwb2,2); ?>%</b></td>
...
<!--akhir petugas-->
	...
	<?
		if ($no!==0){
		?>
		<a href="pmiqc.php?module=cetak_rekap-leucoreduction&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_blue">Download Data Per UTD</a>
		<a href="pmiqc.php?module=cetak_rekap-leucoreduction-all&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_green">Download Semua Data</a>
		<?
		}
	?>
