...
<body>
	...
	<font size="4" color=00008B>LEMBAR DATA ANALISA UJI MUTU <b> ANTI HEMOFILI FAKTOR  (AHF)</b></font><br><br>
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="2" style="width:200px;">UDD PUSAT PMI</th>
  	    <th colspan="2" style="width:300px; height:30px;">LEMBAR DATA ANALISA UJI MUTU ANTI HEMOFILI FAKTOR  (AHF)</th>
	    <th rowspan="2" align="left" style="width:155px;">Halaman <br/>Nomor <br/>Versi <br/>Tanggal Berlaku <br/>Tanggal Kajiulang</th>
	    <th rowspan="2" align="left" style="width:190px;">: 1 dari 1 <br/>: UDDP-PM-L3-053 <br/>: 004 <br/>: 01/07/2020 <br/>: 01/07/2022</th>
	...
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`
		...
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                ...
                WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%AHF' and q.`qc_status` like '$src_hasilqc%' and a.`asal_utd` like '$src_utd%'
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="2">Volume (mL)<br/>(30 - 40 mL) <br/> (derivat WB)</th>
	    <th rowspan="2">Pemeriksaan Koagulasi<br/> Faktor Pembekuan Darah<br/>Faktor FVIII<br/> > 70 IU/unit</th>
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%AHF' 
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%AHF' 
AND q.`volume` >= 30 AND q.`volume` <=40 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%AHF' 
AND q.`vaktorviii` >= 70 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
<!--<td align="center"><b><? echo $hema2; ?>%</b></td>
<td align="center"><b><? echo $leuko2; ?>%</b></td>-->
...
<!--akhir petugas-->
	...
	<?
		if ($no!==0){
		?>
		<a href="pmiqc.php?module=cetak_rekap-ahf&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_blue">Download Data Per UTD</a>
		<a href="pmiqc.php?module=cetak_rekap-ahf-all&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_green">Download Semua Data</a>
		<?
		}
	?>
