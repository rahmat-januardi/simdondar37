...
<body>
	...
	<font size="4" color=00008B>LEMBAR DATA ANALISA UJI MUTU <b> KOMPONEN DARAH PLASMA SEGAR BEKU (FFP)</b></font><br><br>
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="2" style="width:200px;">UDD PUSAT PMI</th>
  	    <th colspan="2" style="width:300px; height:30px;">LEMBAR DATA ANALISA UJI MUTU KOMPONEN DARAH PLASMA SEGAR BEKU (FFP)</th>
	...
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`
		...
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                ...
                WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%FFP' and q.`qc_status` like '$src_hasilqc%' and a.`asal_utd` like '$src_utd%'
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%FFP' 
...
<!--akhir petugas-->
	...
	<?
		if ($no!==0){
		?>
		<a href="pmiqc.php?module=cetak_rekap-ffp&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_blue">Download Data Per UTD</a>
		<a href="pmiqc.php?module=cetak_rekap-ffp-all&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_green">Download Semua Data</a>
		<?
		}
	?>
