...
<body OnLoad="document.cek_form.NoForm.focus();">
	...
	if ($_POST['periksa'] == "1") {
		...
		if (file_exists($myfile0)) {
			$fh0 = fopen($myfile0, 'a') or die("Cant open file");
		} else {
			$fh0 = fopen($myfile0, 'w') or die("Cant open file");
		}
...
if (file_exists($myfile)) { $fh=fopen($myfile,'a') or die ("Cant open file"); } else { $fh=fopen($myfile,'w') or die ("Cant open file"); }*/
		...
		for ($i = 0; $i < count($_POST['no_kantong']); $i++) {
			...
			if ($ckt == 0) {
				...
				$tambah1_sql = "update stokkantong set Status='3', volume='$volume' where  (NoKantong='$NoKantong')";
	...
	if ($_POST['periksa'] == "") { ?>
		<form name="cek_form" method="post" action="<?= $PHPSELF ?>">
			<font size="3" color="black">Masukkan No Formulir --->
				<input type="search" name="NoForm" value="<?= $_GET[noform] ?>" onkeydown="chang(event,this);"
					onchange="cari_form(); this.form.submit();"> Tekan ENTER
			</font>
		</form>
		...
		<form name="periksa" id="periksa" onsubmit="return ok()" method="POST" action=<?= $PHPSELF ?>>
			...
			$check_cross = 0;
			if (isset($_POST[NoForm])) $check_cross = mysql_num_rows(mysql_query("select * from dtransaksipermintaan where NoForm='$_POST[NoForm]' and Status !='B'"));
			$check_batal = mysql_num_rows(mysql_query("select * from dtransaksipermintaan where NoForm='$_POST[NoForm]' and Status='B'"));
			...
			<table class="form" border=0 cellspacing=1 cellpadding=2 width="700px">
				...
				<tr>
					...
					<td>Kode Pasien</td>
					<td class="input"><?= $check_h1[no_rm] ?></td>

				</tr>
				<tr>
					<td>No. reg. RS</td>
					<td class="input"><?= $check_h1[regrs] ?></td>
					<td>Nama Pasien</td>
			...
			<td>Gol. Darah Diminta</td><td class="input"><?= $check_d1[GolDarah] ?></td>
				...
				<tr>
					<td>Sudah Crossmatch</td>
					<td class="input"><?= $check_cross ?> Ktg OK, <?= $check_batal ?> Ktg Incompatible</td>
				...
				<tr>
					...
					<td class="input"><?= $check_d1[Jumlah] - $check_d1[JTitip] ?></td>
					<td>Titip</td>
					<td class="input"><?= $check_d1[JTitip] ?></td>
				</tr>
				<tr>
					<td><b>Masukan No. Kantong</b></td>
					<td class="input"><input name="NoKantong" type="text" id="nokantong" onkeydown="chang(event,this);"
							onchange="focus(document.periksa.NoKantong);cari_kantong('box-table-b');" /> Jika diketik manual
						Tekan ENTER</td>
					<td>HB</td>
					<td class="input"><?= $check_h1[hb] ?></td>
				</tr>
			</table>
