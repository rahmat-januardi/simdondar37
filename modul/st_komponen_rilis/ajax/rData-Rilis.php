...
if (!$user_id) {
    error_log('User tidak ditemukan: ' . $user_id);
...
$sqlTotal = "SELECT COUNT(*) as total 
    ...
    JOIN serahterima s ON sd.dst_notrans = s.hst_notrans
    WHERE s.hst_notrans = '$nT' AND s.hst_pengirim = '$user_id'";
...
$sqlProduk = "SELECT sd.dst_produk, COUNT(*) as jumlah 
    FROM serahterima_detail sd
    JOIN serahterima s ON sd.dst_notrans = s.hst_notrans
    WHERE s.hst_notrans = '$nT' AND s.hst_pengirim = '$user_id'
    GROUP BY sd.dst_produk";
...
$sqlJenis = "SELECT sd.dst_jenisktg, COUNT(*) as jumlah 
    FROM serahterima_detail sd
    JOIN serahterima s ON sd.dst_notrans = s.hst_notrans
    WHERE s.hst_notrans = '$nT' AND s.hst_pengirim = '$user_id'
    GROUP BY sd.dst_jenisktg";
...
$sqlMetoda = "SELECT s.metoda, COUNT(*) as jumlah 
    FROM serahterima_detail d
    JOIN serahterima st ON d.dst_notrans = st.hst_notrans
    JOIN stokkantong s ON d.dst_nokantong = s.noKantong
    WHERE st.hst_notrans = '$nT' AND st.hst_pengirim = '$user_id'
    GROUP BY s.metoda";
