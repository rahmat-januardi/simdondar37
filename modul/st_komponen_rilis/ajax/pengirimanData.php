...
$query = "SELECT * FROM serahterima_detail WHERE `dst_notrans` = '$nT' ORDER BY dst_iddetail DESC";
$result = $dbi->query($query);
if ($result->num_rows > 0) {

    $queryCount = "SELECT 
    COUNT(*) AS total_data, 
    SUM(CASE WHEN dst_sah = 1 THEN 1 ELSE 0 END) AS total_sah 
    FROM serahterima_detail 
    WHERE dst_notrans = '$nT'";

    $resultCount = $dbi->query($queryCount);
    $rowCount = $resultCount->fetch_assoc();

    $jumlahData = isset($rowCount['total_data']) ? $rowCount['total_data'] : 0;
    $jumlahSah = isset($rowCount['total_sah']) ? $rowCount['total_sah'] : 0;

    $checkAllDisabled = ($jumlahSah == $jumlahData) ? 'disabled' : '';

    echo '<table id="tabelPengiriman" class="table table-bordered table-striped table-fixed">';
    echo '<thead>';
    echo '<tr class="text-center align-middle" style="vertical-align: middle !important;">';
    echo '<th class="text-center" rowspan="2"><input type="checkbox" id="checkAll" ' . $checkAllDisabled . '></th>';
    // echo '<th class="text-center" rowspan="2"><input type="checkbox" id="checkAll"></th>';
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $query = "SELECT noKantong, OD, COV, Hasil, notrans, jenisPeriksa, tglPeriksa, nolot, Metode FROM hasilelisa WHERE noKantong LIKE '$tanpaSatelite%' ORDER BY tglPeriksa DESC";
        ...
        echo '<td class="text-center align-middle">
            <input type="checkbox" class="checkbox-row" name="ck_kantong[]" value="' . $row['dst_iddetail'] . '"'
            . ($row['dst_sah'] == 1 ? ' checked disabled' : '') . '>
            </td>';
        echo '<td class="text-center align-middle">' . $no . '</td>';
        echo '<td class="text-center align-middle ' . $extraClass . '"><a href="#" class="kantongDetail" data-id="' . $row['dst_iddetail'] . '">' . $row['dst_nokantong'] . '</a></td>';
        ...
        // echo '<td class="status-kgd text-center align-middle" data-kgd="' . strip_tags($rowKGD['Cocok']) . '">' . $rowKGD['Cocok'] . '</td>';
    ...
    echo '<script>
        window.jumlahData = ' . $jumlahData . ';
        window.jumlahSah = ' . $jumlahSah . ';
        </script>';

    // echo '<button id="hapusTerpilih" class="btn btn-success btn-sm">Proses yang Dipilih</button>';
} else {
