...
*/
			...
			<tr bgcolor="#FFFFFF">
				...
				<td align="center">
					<form name="kirim" method="post"
						action="formulir_donor_PDF3.php?kp=<? echo $dtrans['Kodependonor'] ?>&trans=<? echo $dtrans['NoTrans'] ?>"
						target="_blank">
				...
				<? switch ($_SESSION[leveluser]) {
					case "p2d2s":
				?>
						<td><a href=pmip2d2s.php?module=delhistory&NoTrans=<? echo $dtrans['NoTrans'] ?>&kode=<? echo $dtrans['Kodependonor'] ?>&tgl=<? echo $dtrans['tgl'] ?>><input type=submit name=submit4 value='Hapus'>
							</a></td>
					<?
						break;
					case "aftap":
					?>
						<td><a href=pmiaftap.php?module=delhistory&NoTrans=<? echo $dtrans['NoTrans'] ?>&kode=<? echo $dtrans['Kodependonor'] ?>&tgl=<? echo $dtrans['tgl'] ?>><input type=submit name=submit4 value='Hapus'>
							</a></td>
					<?
						break;
					case "admin":
					?>
						<td><a href=pmiadmin.php?module=delhistory&NoTrans=<? echo $dtrans['NoTrans'] ?>&kode=<? echo $dtrans['Kodependonor'] ?>&tgl=<? echo $dtrans['tgl'] ?>><input type=submit name=submit4 value='Hapus'>
							</a></td>
					<?
						break;
					case "kasir":
					?>
						<td><a href=pmikasir.php?module=delhistory&NoTrans=<? echo $dtrans['NoTrans'] ?>&kode=<? echo $dtrans['Kodependonor'] ?>&tgl=<? echo $dtrans['tgl'] ?>><input type=submit name=submit4 value='Hapus'>
							</a></td>
				<?
	...
	?>
		...
		<table bgcolor="#000012" cellspacing="1" cellpadding="3">
			<tr bgcolor="#FF1000">
				<th colspan=4 color="#DDDDDD"><b>INFO HISTORY KONSELING DONOR</b></th>
				<th>
					<form name=xls method=post action=modul/rekap_sejarah_donor_xls.php>
						<input type=hidden name=q1 value='<?= $q ?>'>
						<input type=hidden name=queryx value='<?= $query ?>'>
						...
						<input type=hidden name=instansix value='<?= $instansi ?>'>
						<input type=hidden name=kendaraanx value='<?= $kendaraan ?>'>
						<input type=hidden name=konseling value='konseling'>
						<input type=submit name=submit2 value='Print History Konseling Donor (.XLS)'>
					</form>
				</th>
			</tr>
		</table>
