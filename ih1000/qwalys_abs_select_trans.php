...
<body>
    ...
    ?>
    <table border=0><tr>
        <td align="left" style="background-color: #ffffff;font-size:24px; color:#0099ff;text-shadow: 1px 1px 1px #000000; font-family:Verdana;">Data Pemeriksaan ABO & ABS  - Erytra Eflexys</td></tr>
    </table>
    ...
    $sql="SELECT *, date(tgl_periksa) as tgl, count(id) as jml from lis_pmi.eflexys
          WHERE jenisperiksa=0 AND stat='1' AND DATE(tgl_periksa)>='$tglawal' and DATE(tgl_periksa)<='$hariini'
          group by notrans";
    //echo $sql;
    ...
    while($tmp=mysql_fetch_assoc($qraw)){
        ...
        <tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
            ...
            <td align="center"><a href="pmikonfirmasi.php?module=ih1000_view_confirm_abd&notrans=<?=$tmp['notrans']?>" class="swn_button_blue">Detail</a></td>
        </tr>
    <?}
    if ($no==0){?>
        <tr class="record">
