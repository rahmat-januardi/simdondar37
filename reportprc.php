...
<body>
	...
	<font size="4" color=00008B>LEMBAR DATA ANALISA UJI MUTU <b> KOMPONEN DARAH MERAH PEKAT (PRC)</b></font><br><br>
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="4" style="width:200px;">UDD PUSAT PMI</th>
  	    <th colspan="2" style="width:300px; height:30px;">LEMBAR DATA ANALISA UJI MUTU KOMPONEN DARAH MERAH PEKAT (PRC)</th>
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%PRC' AND q.`volume` >= 179 AND q.`volume` <= 257 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
<!--akhir petugas-->
	...
	<?
		if ($no!==0){
		?>
		<a href="pmiqc.php?module=cetak_rekap-prc&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_blue">Download Data Per UTD</a>
		<a href="pmiqc.php?module=cetak_rekap-prcall&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_green">Download Semua Data</a>
		<?
		}
	?>
