...
<body>
	...
	<font size="4" color=00008B>LEMBAR DATA ANALISA UJI MUTU <b> KOMPONEN DARAH PLASMA AFERESIS (LP AFERESIS)</b></font><br><br>
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="2" style="width:200px;">UDD PUSAT PMI</th>
  	    <th colspan="2" style="width:300px; height:30px;">LEMBAR DATA ANALISA UJI MUTU KOMPONEN DARAH PLASMA AFERESIS(LP AFERESIS)</th>
	    <th rowspan="2" align="left" style="width:155px;">Halaman <br/>Nomor <br/>Versi <br/>Tanggal Berlaku <br/>Tanggal Kajiulang</th>
	    <th rowspan="2" align="left" style="width:190px;">: 1 dari 1 <br/>: UDDP-PM-L3-052 <br/>: 004 <br/>: 01/07/2020 <br/>: 01/07/2022</th>
	...
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`
		...
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                ...
                WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%LP AFERESIS' and q.`qc_status` like '$src_hasilqc%' and a.`asal_utd` like '$src_utd%'
	...
	<table border=1 cellpadding=4  style="border-collapse:collapse">
	    ...
	    <th rowspan="2">Volume (mL)<br/>(Vol. Tercatat &plusmn 10 %)</th>
	    <th rowspan="2">Pemeriksaan Koagulasi<br/> Faktor Pembekuan Darah<br/>Faktor FVIII<br/> > 0,7 IU/mL</th>
	...
	$sql="SELECT q.`qcchecker`, q.`qcuser`, q.`qctgl`, q.`jenis`, q.`nokantong`, q.`gol_darah`, q.`RhesusDrh`, q.`tglaftap`,
        q.`kadaluwarsa`, q.`berat_isi`, q.`volume`, q.`ph`, q.`swirling`, q.`hemoglobin`, q.`aerob`, q.`anaerob`, 
	q.`anaerob` , a.`asal_utd`, a.`tgl`, s.`nama`, a.`tgl_pengolahan`, q.`trombosit`, q.`leukosit`, q.`vaktorviii`
                FROM `qc` q
                LEFT JOIN `registrasi_qc` a on q.`nokantong`=a.`nokantong`
		LEFT JOIN `stokkantong` b on q.`nokantong`=b.`noKantong`
                LEFT JOIN utd s ON a.`asal_utd` = s.id
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%LP AFERESIS' 
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%LP AFERESIS' 
AND q.`volume` >= 100 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%LP AFERESIS' 
AND q.`vaktorviii` >= 0.7 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%LP AFERESIS' 
AND q.`trombosit` <= 38.88 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
$hema1=mysql_num_rows($sqlwb8);

//leukosit
$sqlwb9=mysql_query("SELECT * FROM `qc` q LEFT JOIN `registrasi_qc` a on q.`nokantong`=a.`nokantong` 
WHERE DATE(q.`qctgl`)>='$tglawal' AND date(q.`qctgl`)<='$hariini' and q.`produk` like '%LP AFERESIS' 
AND q.`leukosit` <= 0.07 and a.`asal_utd` like '$src_utd%'") or die(mysql_error());
...
<!--<td align="center"><b><? echo $aerobwb; ?>%</b></td>
<td align="center"><b><? echo $anaerobwb; ?>%</b></td>-->
...
<table align="right" border=1 cellpadding=4  style="border-collapse:collapse">
        <tr style="background-color:mistyrose; font-size:12px; color:#000000;">
	    <th align="left">Catatan : *Standar Kelulusan* <br/>Kadar Leukosit = < 0,07 x 10<sup>9</sup> /L
	    <br/>Kadar Trombosit = < 38,88 x 10<sup>9</sup></th>
	</tr><br>
</table>
...
<!--akhir petugas-->
	...
	<?
		if ($no!==0){
		?>
		<a href="pmiqc.php?module=cetak_rekap-lpaferesis&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_blue">Download Data Per UTD</a>
		<a href="pmiqc.php?module=cetak_rekap-lpaferesis-all&tgl1=<?=$tglawal?>&tgl2=<?=$hariini?>&stts=<?=$status?>&ptgs=<?=$petugas?>&hasil=<?=$src_hasilqc?>&utd=<?=$src_utd?>" class="swn_button_green">Download Semua Data</a>
		<?
		}
	?>
