<?
$qlastdonasi="select  `KodePendonor`, `NoKantong`, `Tgl`
                  from htransaksi where length(`NoKantong`)>5 and
                  `KodePendonor`='$kodependonor' and `Tgl` < '$tglaftap' ORDER BY `Tgl` DESC";
$qlastdonasi1=mysql_fetch_assoc(mysql_query($qlastdonasi));
$kantonglast=$qlastdonasi1['NoKantong'];
$donasisebelumya=0;
if ($kantonglast==''){
    ?><font size="4" color=red><br><b>Tidak Ada Pemeriksaan Donasi sebelumnya</b></font><br><br><?
} else {
    ?><font size="4" color=00008B><br>Data Pemeriksaan Donasi sebelumnya, Tanggal : <?=$qlastdonasi1['Tgl']?>, No Kantong : <?=strtoupper($qlastdonasi1['NoKantong'])?></font><br><br><?
}
//DATA PEMERIKSAAN IMLTD ELISA SEBELUMNYA==========================================================================================
$sq_elisa1=mysql_query("SELECT `id`, `noKantong`, `OD`, `COV`, `notrans`,
    case
    ...
    FROM `hasilelisa` WHERE `noKantong`='$kantonglast' order by `id`");

$var_imltd_old='0';
?>
<font size="2" color=black><b>Pemeriksaan IMLTD metode ELISA/CHLIA</font></b>
<table cellpadding=3 cellspacing=3 width="100%">
    ...
    <?
    $no="0";
    while ($imltd1=mysql_fetch_assoc($sq_elisa1)){$no++;
        if (($imltd1[Hasil]=='Reaktif') or ($imltd1[Hasil]=='Grayzone')){$var_imltd_old='1';}
        $sq_reagen1=mysql_fetch_assoc(mysql_query("SELECT `Nama`, `noLot`, `tglKad`  FROM `reagen` WHERE kode='$imltd1[noLot]'"));
	if ($sq_reagen1[noLot]==""){
		$sq_reagen1=mysql_fetch_assoc(mysql_query("SELECT `Nama`, `noLot`, `tglKad`  FROM `reagen` WHERE noLot='$imltd1[noLot]'"));
	}
        ?>
        <tr style="font-size:13px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight';" onMouseOut="this.className='normal';">
            <td class=input><?=$imltd1[id]?></td>
            <td class=input><?=$imltd1[noKantong]?></td>
            <td class=input><?=$imltd1[notrans]?></td>
            <td class=input><?=$imltd1[tglPeriksa]?></td>
            <td class=input><?=$imltd1[Parameter]?></td>
            <td class=input><?=$imltd1[OD]?></td>
            <?if($imltd1[Hasil]=="Non Reaktif"){?><td align="left"><font color="black"><?=$imltd1[Hasil]?></font></td><?} else {?><td align="left"><font color="red"><?=$imltd1[Hasil]?></font></td><?}?>
            <td class=input><?=$sq_reagen1[Nama]?></td>
            <td class=input><?=$sq_reagen1[noLot]?></td>
            <td class=input><?=$sq_reagen1[tglKad]?></td>
            <td class=input><?=$imltd1[dicatatOleh]?></td>
            <td class=input><?=$imltd1[dicekOleh]?></td>
            <td class=input><?=$imltd1[DisahkanOleh]?></td>
        </tr>
    <?}
    if ($no=="0"){
...
$sq_rapid=mysql_query("SELECT `id`, `NoTrans`, `noKantong`, `Kontrol`,
						...
						FROM `drapidtest` WHERE `noKantong`='$kantonglast' order by `id`");
?>
<font size="2" color=black><b>Pemeriksaan IMLTD metode Rapid</font></b>
<table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
    ...
    while ($imltd_r=mysql_fetch_assoc($sq_rapid)){$no++;
        if (($imltd_r[Hasil]=='Reaktif') or ($imltd_r[Hasil]=='Grayzone')){$var_imltd_old='1';}
...
$sq_nat="SELECT `id`, `date_transfer`, `sample_id`, `interpretation`, `protocol`, `run_number`, `date`, `flag`, `internal_control_rlu`,
        ...
        `tgl_konfirmasi`, `userkonfirmasi` FROM `imltd_procleix_raw` WHERE `sample_id`='$kantonglast'";
$sql_nat=mysql_query($sq_nat);
//echo "$sq_nat";
?>
<font size="2" color=black><b>Pemeriksaan IMLTD metode NAT</font></b>
<table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
    <tr style="background-color:mistyrose; font-size:12px; color:#000000;">
        <th rowspan="2">ID</th>
...
<table class="list" border=1 cellpadding="2" cellspacing="2" width="100%" style="border-collapse:collapse">
    ...
    <?
    $a=mysql_query("select * from dkonfirmasi where  NoKantong='$kantonglast' order by NoKonfirmasi ASC");
    $no=1;
    while($a_dtransaksipermintaan=mysql_fetch_assoc($a)){?>
        <tr style="font-size:12px; color:#000000; font-family:Verdana;" onMouseOver="this.className='highlight'" onMouseOut="this.className='normal'">
            ...
            if ($a_dtransaksipermintaan[Cocok]=='1'){$var_kgd_old='1';}
            $cocok1='-';
