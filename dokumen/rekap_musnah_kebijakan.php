...
if (isset($_GET['export_excel'])) {
    ...
    header('Content-Disposition: attachment; filename="dokumen_kebijakan_ditarik.xls"');
    ...
    $exportQuery = mysql_query("select * from kebijakan where aktif='1' order by kontrol");
    $today = date('Y-m-d');

    echo '<meta charset="UTF-8">';
    echo '<style>body{font-family:Calibri,Arial,sans-serif;color:#1f2937}h2{color:#1f4e78;font-size:18pt}table{border-collapse:collapse;width:100%;font-size:11pt}th{background-color:#1f4e78;color:#ffffff;font-weight:bold;text-align:center;vertical-align:middle;padding:9px;border:1px solid #9eafbf}td{padding:7px;border:1px solid #b8c4ce;vertical-align:middle}tr:nth-child(even) td{background-color:#f3f6f8}.center{text-align:center}.status-valid{background-color:#deb887;font-weight:bold;text-align:center}.status-expired{background-color:#b22222;color:#ffffff;font-weight:bold;text-align:center}.status-review{background-color:#ffff00;font-weight:bold;text-align:center}</style>';
    echo '<h2>Daftar Dokumen Kebijakan Yang Telah Ditarik</h2>';
    echo '<table>';
    echo '<tr><th>No</th><th>Bidang</th><th>Judul Dokumen</th>'
        . '<th>Identitas Dokumen Sebelumnya</th><th>Tingkat Dokumen</th>'
        . '<th>No. Kontrol Dokumen</th><th>Periode Kaji Ulang (bln)</th><th>No. Versi</th>'
        . '<th>Tanggal Disahkan</th><th>Tanggal Berlaku</th><th>Tanggal Kaji Ulang</th>'
        . '<th>Tanggal Penarikan</th><th>Keterangan Penarikan</th><th>File</th>'
        . '<th>Masih Berlaku</th><th>Habis Masa Kadaluwarsa</th><th>Peninjauan Kembali</th></tr>';
    ...
    while ($exportData = mysql_fetch_array($exportQuery)) {
        ...
        $peninjauanKembali = $exportData['tgl_notif'] <= $today ? 'Waktunya Peninjauan Kembali' : '-';
        ...
        echo '<tr><td class="center">' . $exportNo . '</td>'
            ...
            . '<td class="center">' . export_excel_escape($exportData['kontrol']) . '</td>'
            ...
            . '<td class="center">' . export_excel_date($exportData['tgl_pemusnahan']) . '</td>'
            . '<td>' . export_excel_escape($exportData['ket']) . '</td>'
            . '<td>' . export_excel_escape($exportData['fileku']) . '</td>'
            . '<td class="' . $validClass . '">' . $masihBerlaku . '</td>'
            . '<td class="' . $expiredClass . '">' . $habisMasaBerlaku . '</td>'
            . '<td class="' . $reviewClass . '">' . $peninjauanKembali . '</td></tr>';
    }
    echo '</table>';
    exit;
}
...
<body>
    ...
    <a href="?export_excel=1"><button class="button1" style="background-color:#3e8e41;">Export Excel</button></a>
    <br><br>
    <p align="center"><b> Daftar Dokumen Kebijakan Yang Telah Ditarik</b></p>
    <table>
        <thead>
            <tr>
                <th rowspan="2">No</th>
                ...
                <th rowspan="2">Keterangan Penarikan</th>
        ...
        <tbody id="myTable">
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <div align="center"><?php echo $data['kontrol']; ?></div>
                    </td>
                    <td>
                    ...
                    <td>
                        <div align="center"><?php echo format_indo($data['tgl_pemusnahan']); ?></div>
                    </td>
                    <td>
                        <div align="center"><?php echo $data['ket']; ?></div>
                    </td>
                    <td><a href="download.php?filename=<?= $data['fileku'] ?>"><?php echo $data['fileku']; ?></a></td>
                    <td class="<?php echo $data['tgl_peninjauan'] >= $today ? 'status-valid' : ''; ?>">
                        <div align="center"><?php echo strip_tags($pengerjaan1); ?></div>
                    </td>
                    <td class="<?php echo $data['tgl_peninjauan'] <= $today ? 'status-expired' : ''; ?>">
                        <div align="center"><?php echo strip_tags($pengerjaan2); ?></div>
                    </td>
                    <td class="<?php echo $data['tgl_notif'] <= $today ? 'status-review' : ''; ?>">
                        <div align="center"><?php echo strip_tags($pengerjaan3); ?></div>
                    </td>
