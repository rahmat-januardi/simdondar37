...
if (isset($_POST['submit']) && !empty($_POST['bidang'])) {
  ...
  $queries = array(
    array(
      'title' => 'KEBIJAKAN',
      'query' => "SELECT * FROM kebijakan WHERE aktif='0' AND kontrol LIKE '%$bidang%' ORDER BY RIGHT(kontrol,3)",
      'withFormat' => false
    ),
    array(
      'title' => 'SPO',
      'query' => "SELECT * FROM pks WHERE aktif='0' AND kontrol2 LIKE '%$bidang%' ORDER BY kontrol2",
      'withFormat' => false
    ),
    array(
      'title' => 'IK',
      'query' => "SELECT * FROM ik WHERE aktif='0' AND kontrol2 LIKE '%$bidang%' ORDER BY kontrol2",
      'withFormat' => false
    ),
    array(
      'title' => 'IK ALAT',
      'query' => "SELECT * FROM ika WHERE aktif='0' AND kontrol2 LIKE '%$bidang%' ORDER BY kontrol2",
      'withFormat' => false
    ),
    array(
      'title' => 'DOKUMEN PENDUKUNG',
      'query' => "SELECT * FROM pendukung WHERE aktif='0' AND kontrol2 LIKE '%$bidang%' ORDER BY kontrol2",
      'withFormat' => false
    ),
    array(
      'title' => 'FORMULIR',
      'query' => "SELECT nama1, bidang, kontrol2, GROUP_CONCAT(fileku) AS semua_file FROM formulir WHERE aktif='0' AND kontrol2 LIKE '%$bidang%' GROUP BY nama1, kontrol2 ORDER BY kontrol2",
      'withFormat' => true
    ),
    array(
      'title' => 'DOKUMEN EKSTERNAL',
      'query' => "SELECT nama1, bidang, kontrol2, GROUP_CONCAT(fileku) AS semua_file FROM eksternal WHERE aktif='0' AND kontrol2 LIKE '%$bidang%' GROUP BY nama1, kontrol2 ORDER BY kontrol2",
      'withFormat' => true,
      'isEksternal' => true
    )
  );
  ...
  <form method="post">
    ...
    <select name="bidang" id="bidangSelect" style="width:300px;">
      ...
      while ($data = mysql_fetch_array($hasil)) {
        echo "<option value='$data[kode_bidang]'>$data[bidang]</option>";
      }
      ?>
    </select>
